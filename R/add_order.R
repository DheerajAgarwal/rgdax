...
#'
#' @description This is an auth based function. The users can place different types of orders like \code{"limit"}, \code{"stop"} or \code{"market"}. Orders will be placed succccesffully only if there is sufficient funds. each order will result in a hold and the details of the hold can be tracjked using \code{\link{holds}}. Margin Orders are not supported.
#'
